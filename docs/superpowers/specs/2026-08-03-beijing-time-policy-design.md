...
- 欢迎页保留主按钮“创建我的第一个习惯”，并在其下增加次级按钮“暂时跳过”。
- 点击“暂时跳过”立即进入人生工作台，不创建任何占位习惯。
- DataStore 新增本地布尔值 `onboardingCompleted`。创建首个习惯完成或点击跳过时均写入 `true`。
- 首次启动目的地由“是否完成引导”与“是否已有习惯”共同决定：已有习惯或引导已完成时进入工作台，否则显示欢迎页。
- 引导完成状态是设备级 UI 状态，不加入数据备份；导入数据不会将它重置。
- 跳过后的空工作台继续显示现有新建习惯入口。
...
- 新用户点击跳过后立即进入工作台，重启应用后仍进入工作台；未选择跳过的新用户仍显示欢迎页。
- 完整单元测试与 APK 构建通过。
