...
& 'D:\MySoftware\Android\AndroidSdk\platform-tools\adb.exe' install 'Habit-0.2.0-debug.apk'
```

如果手机上已经安装同一包名的旧版，可保留本地数据覆盖升级：

```powershell
& 'D:\MySoftware\Android\AndroidSdk\platform-tools\adb.exe' install -r 'Habit-0.2.0-debug.apk'
```
...
1. 将 `Habit-0.2.0-debug.apk` 复制到手机。
2. 在文件管理器中点按 APK。
...
6. 在“主题与设置”中可切换五套内置主题。

## 本地数据说明

Habit 0.2.0 不需要账号，习惯、分类、打卡和主题均只保存在本机，不会上传或同步到云端。从 0.1.0 升级时必须直接覆盖安装；不要先卸载旧版，也不要清除应用数据。应用已禁用 Android 系统云备份，并通过提取规则排除 Android 12 及以上的设备间数据迁移；卸载应用、清除应用数据或刷机会永久删除这些记录，当前版本不提供备份恢复功能。
