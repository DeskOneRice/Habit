...
- APK 文件名：`Habit-0.6.0-release.apk`
- Android 包名：`com.habit.app`
- 版本：`versionCode 12` / `versionName 0.6.0`
...
2. 查看设备并从 0.5.1 覆盖安装 0.6.0：

```powershell
& 'D:\MySoftware\Android\AndroidSdk\platform-tools\adb.exe' devices -l
& 'D:\MySoftware\Android\AndroidSdk\platform-tools\adb.exe' install -r 'artifacts\Habit-0.6.0-release.apk'
```
...
```powershell
& 'D:\MySoftware\Android\AndroidSdk\platform-tools\adb.exe' install 'artifacts\Habit-0.6.0-release.apk'
```

## 直接在手机上安装

1. 将 `Habit-0.6.0-release.apk` 复制到手机。
2. 在文件管理器中点按 APK。
...
4. 切到“功能绑定”，为“综合周报”和“图片热量估算”分别选择模型；不需要时选择“暂不绑定”。
5. 综合周报会在发送结构化摘要前再次确认；图片热量估算要求餐食已有照片，并在选择照片后再次确认发送。
...
Habit 0.6.0 不需要账号，业务数据默认保存在本机，不会自动同步。只有用户主动确认生成周报或估算照片时，应用才会向已绑定模型发起网络请求。应用内默认将备份写入 `内部存储 / Download / Habit / 数据备份`，也可在设置中选择其他文件夹并自动迁移已有备份。导入时可选择“合并导入”或“完全替换”；合并冲突按更新时间和既定规则处理。schema 5 备份可包含 AI 模型的非秘密元数据、功能绑定、周报及热量依据，但不包含 API Key。
